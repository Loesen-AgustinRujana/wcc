...
const Table: FunctionComponent<Props> = ({
  ...
  const previous = () => {
    ...
    axios({
      method: "get",
      url: `${"http://api.catalogue.wagentekk.com"}/vehicles?page=${page - 1}?${query}`,
      // headers: {
      ...
      .then((res) => {
        ...
        setPositions(res.data.vehicles);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const next = () => {
    ...
    axios({
      method: "get",
      url: `${"http://api.catalogue.wagentekk.com"}/vehicles?page=${page + 1}`,
      // headers: {
      //   Authorization: `Bearer ${accessToken}`,
      // },
      data: "",
